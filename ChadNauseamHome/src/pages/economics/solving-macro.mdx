...
Every year the government should figure out the income of every American for that year. The Fed's goal is _path targeting_ such that the average income rises along a path of increasing about 4% each year.

There's a pretty straightforward way to do this: set up two bonds, one that pays out of the fed undershoots this path, and one that pays out if the fed overshoots the path. Then: Every time a bond is bought that says the fed will undershoot, automatically increase the money supply. And every time a bond is bought that says the Fed will overshoot, decrease the money supply. What could be simpler?
