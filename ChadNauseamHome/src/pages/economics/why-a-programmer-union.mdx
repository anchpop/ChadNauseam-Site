...
Let's set #2 aside for now. How is all the money from dues going to be spent? Simple. Programmers can donate money, out of their own pockets, to anyone they want. In return, the union will use dues as a subsidy pool, to "match" their donation according to [the quadratic funding system](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3243656):
...
Here's the feedback loop: we set aside some money from union dues (let's say 25%) into a separate subsidy pool.<Note>If you don't want your money going into the union pool, maybe you can elect for it to be donated to Givewell instead. Also, maybe it could be funded by unspent money from the main subsidy pool, if there is any.</Note> This pool is reserved for donations to UGPL-licensed projects. So if there's initially no UGPL-licensed projects, the first person who writes one and gets a donation would receive the entire subsidy pool. This would create a huge incentive for people to license their code as UGPL (which means also licensing it at GPL, since UGPL is a dual-license).
