...
The obvious solution is to have one person compute it and ask them, but then you'd have to trust that they weren't lying. The trick here is to use a technology called ZK-snarks or ZK-starks (among others). These let you run an arbitrary computation and compute its result along with a _cryptographic proof_ that that result really is the result of running the computation. The beautiful thing is that this proof is small, a few bytes (for snarks) to a few kilobytes (to starks), and can be checked in a snap. So you can let anyone calculate `(balance, proof)`, and as long as one person is willing to do that, no one else needs to redo their work.
...
It's not theoretically necessary for every node to store the full ledger, and if you want to support a lot of operations per second you quickly run into storage problems. That's why one of the biggest research areasis _data sharding_, which allows you to split up the ledger between multiple nodes.
...
But luckily, we now have lots of scalable proof-of-stake consensus algorithms for _deterministic_ finality. That's not an easy problem to solve, but it's solved now. Once Ethereum switches to proof of stake (any day now...), they'll have pretty-good finality after just one block (and there will be a new block every 12 seconds), and guaranteed<Note> Guaranteed so long as less than 33% of the nodes are dishonest, that is.</Note> finality after two blocks. And proof of stake has other advantages too, like having basically no energy consumption and not driving up the cost of graphics cards.
