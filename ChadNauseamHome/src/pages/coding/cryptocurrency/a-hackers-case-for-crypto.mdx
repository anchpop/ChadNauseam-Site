...
If you want to support a lot of operations per second you quickly run into storage problems if you ask every node to store the full ledger. That's why one of the biggest research area is _data sharding_, which allows you to split up the ledger between multiple nodes.
