...
1. Games can be tested manually by QA teams, whose time is less expensive than developers', so hiring outside help to manually test games makes some sense. 
But it isn't strictly superior – the benefit of automated testing is that it scales in O(1) with respect to number of code changes, while manually testing is O(n), so only automated tests can e.g. be included in CI. 
...
For example, you could have a system that queries for all entities that have a position component and a velocity component, and then iterate over them to modify the position based on the velocity. 
In 
...
[Writing Tests in Bevy, by jroddev](https://blog.jroddev.com/bevy-tests/). A great post that covered many things I wish I'd covered here! 
