...
Put explicitly: If you write `x = foo()`, that means you should be able to replace any occurrence of `x` with `foo()` without changing the meaning. No-mutation and no-side-effects follow from this idea.
...
I'm going to be using a Rust-inspired syntax for this post, where you can write something like
...
To describe an enum with four possible values. So you could create a value of type `Message` by writing `Move {x=3, y=5}` or something like that.
...
3. Let's extend our type one final time.
   ...
   enum ActionValue {
     Done,
     Print {to_print: String, then: ActionValue},
     Input {to_print: String, then: String -> ActionValue}
   }
   ```

   Now `Input`'s `then` parameter takes a function, instead of a plain action value. Which means we can write stuff like this:
   ...
   Unlike before, `Input`'s `then` doesn't store an action value, it stores a function that takes a string and returns an action value. Whatever the user types will be passed to that function, and then the resulting action value will be executed.
...
Monads really are just a convenient way to build up action values. In Haskell, the end result is that you can write code like this:
