...
1. I actually have bigger and badder ideas for row. What I really want is a library that's like a mix of Pandas and [differential-dataflow](https://github.com/TimelyDataflow/differential-dataflow) for storing tables of product types. Its type would be something like `Database(r)` where `r` is a row.
...
1. Another area that needs another look is making dependent records and rows work together. If you have any ideas for this let me know.

1. How should product access work? `x::y` to access `y` field from product `x` makes sense... but it would be nice if `y` was an optic so you could do cool optic stuff.
...
Dependent types are one promising avenue, and I think they're a good idea, but they're starting to become well explored. I want to bring attention to _refinement types_, which I think are even more exciting. <Note>Refinement types and dependent types are orthogonal, so you can have one or the other or both or neither if you want.</Note>
...
1. `thats` is a keyword that indicates that on the left is a base type, and the right is a refinement.

1. `> 0` is a function that you're proving will return true when provided a value of this type - this function is also known as a predicate.
...
And the compiler could _automatically_ infer out that `_` is a stand-in for `>= 0`.
