...
3. Functional debugging.
   ...
   This is something that's always annoyed me - debugging in functional language is usually _so bad_, but in principle it could actually be much better than in any imperative language. Imagine this preudocode:

   ```reason
   let handle_request =
      (request, path, old_state) => {
        let f = path.get_handler;
        let request = request.of_json;
        switch (old_state.f(request)) {
        | Ok((new_state, response)) =>
          (new_state, Response200 response.to_json);
        | Error(err) =>
          print_error(err);
          (new_state, (Response500 "Internal server error!"));
        };
      };
   ```
