...
In Proof of Work, we do two things.

1. We make it so the only valid operations on the blockchain are very computationally expensive.

   They're so expensive that only one valid operation is discovered every 10 minutes, despite millions of miners collectively using [15 Gigawatts](https://cbeci.org/)<Note numbered>At the time of this writing.</Note> of electrical power to discover valid operations as fast as possible.

1. We make it so the set of valid operations is unique for every possible state.

Both of these are solved with something called a _hash function_. I can't do a better job at explaining it than [this 3blue1brown video](https://www.youtube.com/watch?v=bBC-nXj3Ng4&vl=en), so I recommend you just go watch that.
