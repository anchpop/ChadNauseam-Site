...
`PUSH int 2; PUSH int 2;`: Pushes two ints (`2` and `2`, to be specific) onto the stack. The stack now contains `2,2`

`ADD;`: Pops the top two values off the stack, adds them together, then pushes the result back onto the stack. The stack now contains `4`.

`PUSH int 4;`: This pushes the int `4` onto the stack. The stack now contains `4, 4`.

`ASSERT_CMPEQ;`: This pops the top two items off the stack and asserts that they are the same. If they aren't, the contract will crash.

Michelson's semantics are specified [here](https://tezos.gitlab.io/008/michelson.html#language-semantics).
...
I'm here to work on the protocol, so the most relevant folder is `proto_alpha`. The rest are just old versions of the protocol that you don't have to worry about.
