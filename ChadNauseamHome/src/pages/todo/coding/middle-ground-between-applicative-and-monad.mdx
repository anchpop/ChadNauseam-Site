...
Categories are a pretty simple concept for such an intimidating term. Categories are anything that look like functions – they're only required to have `id` and `(.)`.
...
So if you have function-like things, categories let you compose them and call `id` on them. But it's kind of interesting t
...
Arrows enhance applicatives with the ability to represent arbitrary dependency.

<CenteredImg src={arrow_dataflow} style={{ maxWidth: "21em" }} />
