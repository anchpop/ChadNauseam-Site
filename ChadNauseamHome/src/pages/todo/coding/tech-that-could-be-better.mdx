...
1. TVs
   ...
   1. This is actually already possible with HDMI CEC, but no TV I've ever used supports it: why do I still have to remember HDMI1, HDMI2, etc. when the device plugged in could tell the TV its name and have the TV display that?

1. The Internet

   1. Services like YouTube are awesome because it's free to upload and there's no charge to watch. But they're not awesome because they show ads and you become dependent on YouTube's consent to continue communicating with your audience. Much better would be a system where you pay to host your own videos on your own platform (using some off-the-shelf code you can throw up on Cloudflare or whatever else you want.) If you want to do advertising, the service could run an automated auction to decide which ads to embed at the beginning of your videos. Then you submit your API link to some online directories, and users have an app that crawls those directories and shows them all the unwatched videos from people they subscribe to. That's quite similar to how podcasts work, so there's some prior art for this, but I don't see why this principle shouldn't be applied to every platform. (Things like recommendation algorithms could be implemented through some kind of FLOC-type system.)
   ...
   1. When I want to embed code into a webpage, for a tutorial or something like that, I'd ideally like to make good use of the horizontal space my users have. If they're on a desktop, I might want to show something like this:

      ```
      hello_world = foo().bar().baz().bingo().bongo().wham().bam().thankyou().maam().dostuff(x);
      ```

      But on a mobile device, that might introduce an ugly scrollbar, so in that case I'd rather show something like this:
      ...
      It would be nice if there were a simple way to, at build time, run a formatter over all my code samples and generate versions for 180 character screen width, 120 character, and 80 character, then have the website pick the appropriate one with CSS media queries.
